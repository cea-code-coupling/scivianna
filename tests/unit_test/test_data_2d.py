...
def create_data2d(cell_ids: list, values: list, default_color: Tuple[int, int, int] = (100, 100, 100), default_edge_color: Tuple[int, int, int] = (50, 50, 50)) -> Data2D:
    ...
    data.cell_ids = np.array(cell_ids)
    data.cell_values = np.array(values, dtype=float)
    # Colors are numpy arrays of shape (n, 4) with RGBA values
    n = len(cell_ids)
    color_rgba = np.array([default_color[0], default_color[1], default_color[2], 255])
    edge_color_rgba = np.array([default_edge_color[0], default_edge_color[1], default_edge_color[2], 255])
    data.cell_colors = np.tile(color_rgba.reshape(1, 4), (n, 1))
    data.cell_edge_colors = np.tile(edge_color_rgba.reshape(1, 4), (n, 1))
    data.polygons = []
    return data


def assert_array_equal_safe(actual, expected, attr_name: str):
    """Helper to compare numpy arrays or lists, handling NaN values."""
    actual_arr = np.asarray(actual, dtype=float)
    expected_arr = np.asarray(expected, dtype=float)
    
    # For numeric arrays, handle NaN values
    assert actual_arr.shape == expected_arr.shape, f"Shape mismatch in {attr_name}: {actual_arr.shape} vs {expected_arr.shape}"
    mask = ~np.isnan(expected_arr)
    np.testing.assert_array_equal(actual_arr[mask], expected_arr[mask], err_msg=f"Mismatch in {attr_name} for non-NaN values")
    nan_mask = np.isnan(expected_arr)
    assert np.all(np.isnan(actual_arr[nan_mask])), f"Expected NaN values in {attr_name} but got actual values"


def assert_array_equal_strict(actual, expected, attr_name: str):
    """Helper to compare numpy arrays strictly (no NaN handling)."""
    actual_arr = np.asarray(actual)
    expected_arr = np.asarray(expected)
    np.testing.assert_array_equal(actual_arr, expected_arr, err_msg=f"Mismatch in {attr_name}")
...
class TestReorderData2D:
    def test_reorder_same_order(self):
        ...
        assert list(result.cell_ids) == [1, 2, 3]
        assert_array_equal_safe(result.cell_values, [10.0, 20.0, 30.0], "cell_values")
    ...
    def test_reorder_different_order(self):
        ...
        assert list(result.cell_ids) == [3, 1, 2]
        assert_array_equal_safe(result.cell_values, [30.0, 10.0, 20.0], "cell_values")

    def test_reorder_subset(self):
        """Test reordering with a subset of cell_ids."""
        data = create_data2d([1, 2, 3], [10.0, 20.0, 30.0])
        result = Data2D._reorder_data2d_to_match(data, [2, 1])
        assert list(result.cell_ids) == [2, 1]
        assert_array_equal_safe(result.cell_values, [20.0, 10.0], "cell_values")
    ...
    def test_reorder_superset(self):
        ...
        result = Data2D._reorder_data2d_to_match(data, [1, 2, 3])
        assert list(result.cell_ids) == [1, 2, 3]
        assert_array_equal_safe(result.cell_values, [10.0, 20.0, np.nan], "cell_values")
    ...
    def test_reorder_string_cell_ids(self):
        ...
        assert list(result.cell_ids) == ["c", "a"]
        assert_array_equal_safe(result.cell_values, [3.0, 1.0], "cell_values")
...
class TestAdd:
    def test_add_scalar(self):
        ...
        result = data + 5.0
        assert list(result.cell_ids) == [1, 2, 3]
        assert_array_equal_safe(result.cell_values, [15.0, 25.0, 35.0], "cell_values")
    ...
    def test_add_same_cell_ids(self):
        ...
        result = data1 + data2
        assert list(result.cell_ids) == [1, 2, 3]
        assert_array_equal_safe(result.cell_values, [11.0, 22.0, 33.0], "cell_values")
    ...
    def test_add_different_order(self):
        ...
        assert_array_equal_safe(result.cell_values, [13.0, 22.0, 31.0], "cell_values")
    ...
    def test_add_different_cell_ids(self):
        ...
        assert_array_equal_safe(result.cell_values, [np.nan, 22.0, np.nan], "cell_values")
    ...
    def test_add_in_place(self):
        ...
        data = create_data2d([1, 2, 3], [10.0, 20.0, 30.0])
        data += 5.0
        assert_array_equal_safe(data.cell_values, [15.0, 25.0, 35.0], "cell_values")
...
class TestSub:
    def test_sub_scalar(self):
        ...
        result = data - 5.0
        assert list(result.cell_ids) == [1, 2, 3]
        assert_array_equal_safe(result.cell_values, [5.0, 15.0, 25.0], "cell_values")
    ...
    def test_sub_same_cell_ids(self):
        ...
        result = data1 - data2
        assert list(result.cell_ids) == [1, 2, 3]
        assert_array_equal_safe(result.cell_values, [9.0, 18.0, 27.0], "cell_values")
    ...
    def test_sub_different_order(self):
        ...
        assert list(result.cell_ids) == [1, 2, 3]
        # After reordering data2: values should be [3.0, 2.0, 1.0]
        assert_array_equal_safe(result.cell_values, [7.0, 18.0, 29.0], "cell_values")
    ...
    def test_sub_different_cell_ids(self):
        ...
        # Union order: [1, 2, 3]
        assert list(result.cell_ids) == [1, 2, 3]
        assert_array_equal_safe(result.cell_values, [np.nan, 18.0, np.nan], "cell_values")
    ...
    def test_rsub(self):
        ...
        result = 100.0 - data
        assert list(result.cell_ids) == [1, 2, 3]
        assert_array_equal_safe(result.cell_values, [90.0, 80.0, 70.0], "cell_values")

    def test_sub_in_place(self):
        """Test in-place subtraction (-=)."""
        data = create_data2d([1, 2, 3], [10.0, 20.0, 30.0])
        data -= 5.0
        assert_array_equal_safe(data.cell_values, [5.0, 15.0, 25.0], "cell_values")
...
class TestMul:
    def test_mul_scalar(self):
        ...
        result = data * 2.0
        assert list(result.cell_ids) == [1, 2, 3]
        assert_array_equal_safe(result.cell_values, [20.0, 40.0, 60.0], "cell_values")
    ...
    def test_mul_same_cell_ids(self):
        ...
        result = data1 * data2
        assert list(result.cell_ids) == [1, 2, 3]
        assert_array_equal_safe(result.cell_values, [20.0, 60.0, 120.0], "cell_values")
    ...
    def test_mul_different_order(self):
        ...
        assert list(result.cell_ids) == [1, 2, 3]
        # After reordering data2: values should be [3.0, 2.0, 1.0]
        assert_array_equal_safe(result.cell_values, [30.0, 40.0, 30.0], "cell_values")
    ...
    def test_mul_different_cell_ids(self):
        ...
        # Union order: [1, 2, 3]
        assert list(result.cell_ids) == [1, 2, 3]
        assert_array_equal_safe(result.cell_values, [np.nan, 40.0, np.nan], "cell_values")
    ...
    def test_mul_int_scalar(self):
        ...
        result = data * 3
        assert list(result.cell_ids) == [1, 2, 3]
        assert_array_equal_safe(result.cell_values, [30.0, 60.0, 90.0], "cell_values")
    ...
    def test_mul_in_place(self):
        ...
        data = create_data2d([1, 2, 3], [10.0, 20.0, 30.0])
        data *= 2.0
        assert_array_equal_safe(data.cell_values, [20.0, 40.0, 60.0], "cell_values")
...
class TestDiv:
    def test_div_scalar(self):
        ...
        result = data / 2.0
        assert list(result.cell_ids) == [1, 2, 3]
        assert_array_equal_safe(result.cell_values, [5.0, 10.0, 15.0], "cell_values")
    ...
    def test_div_same_cell_ids(self):
        ...
        result = data1 / data2
        assert list(result.cell_ids) == [1, 2, 3]
        assert_array_equal_safe(result.cell_values, [5.0, 5.0, 5.0], "cell_values")
    ...
    def test_div_different_order(self):
        ...
        assert list(result.cell_ids) == [1, 2, 3]
        # After reordering data2: values should be [3.0, 2.0, 1.0]
        expected = [10.0 / 3.0, 20.0 / 2.0, 30.0 / 1.0]
        assert_array_equal_safe(result.cell_values, expected, "cell_values")
    ...
    def test_div_different_cell_ids(self):
        ...
        # Union order: [1, 2, 3]
        assert list(result.cell_ids) == [1, 2, 3]
        assert_array_equal_safe(result.cell_values, [np.nan, 10.0, np.nan], "cell_values")
    ...
    def test_rdiv(self):
        ...
        result = 20.0 / data
        assert list(result.cell_ids) == [1, 2, 3]
        assert_array_equal_safe(result.cell_values, [10.0, 5.0, 4.0], "cell_values")
    ...
    def test_div_in_place(self):
        ...
        data = create_data2d([1, 2, 3], [10.0, 20.0, 30.0])
        data /= 2.0
        assert_array_equal_safe(data.cell_values, [5.0, 10.0, 15.0], "cell_values")
...
class TestEdgeCases:
    ...
    def test_empty_data2d_add(self):
        ...
        result = data1 + data2
        assert list(result.cell_ids) == [1, 2]
        assert np.isnan(result.cell_values[0])
    ...
    def test_single_cell_operations(self):
        ...
        result = data + 5.0
        assert_array_equal_safe(result.cell_values, [15.0], "cell_values")
    ...
    def test_result_has_default_colors_for_data2d_operation(self):
        ...
        data1.cell_colors = np.array([[100, 100, 100, 255], [150, 150, 150, 255]])
        data2.cell_colors = np.array([[200, 200, 200, 255], [250, 250, 250, 255]])
        result = data1 + data2
        # Result should have white colors for cells and (200, 200, 200, 255) for edges
        expected_colors = np.array([[255, 255, 255, 255], [255, 255, 255, 255]])
        expected_edges = np.array([[200, 200, 200, 255], [200, 200, 200, 255]])
        assert_array_equal_strict(result.cell_colors, expected_colors, "cell_colors")
        assert_array_equal_strict(result.cell_edge_colors, expected_edges, "cell_edge_colors")
    ...
    def test_result_preserves_colors_for_scalar_operation(self):
        ...
        data.cell_colors = np.array([[100, 100, 100, 255], [150, 150, 150, 255]])
        data.cell_edge_colors = np.array([[50, 50, 50, 255], [75, 75, 75, 255]])
        result = data + 5.0
        # Colors are preserved after copy()
        expected_colors = np.array([[100, 100, 100, 255], [150, 150, 150, 255]])
        expected_edges = np.array([[50, 50, 50, 255], [75, 75, 75, 255]])
        assert_array_equal_strict(result.cell_colors, expected_colors, "cell_colors")
        assert_array_equal_strict(result.cell_edge_colors, expected_edges, "cell_edge_colors")
    ...
    def test_original_data_unchanged(self):
        ...
        data = create_data2d([1, 2, 3], [10.0, 20.0, 30.0])
        _ = data + 5.0
        assert_array_equal_safe(data.cell_values, [10.0, 20.0, 30.0], "cell_values")
    ...
    def test_integer_cell_ids(self):
        ...
        result = data1 + data2
        assert list(result.cell_ids) == [1, 2, 3]
        assert_array_equal_safe(result.cell_values, [12.0, 23.0, 31.0], "cell_values")
    ...
    def test_mixed_string_cell_ids(self):
        ...
        result = data1 + data2
        assert list(result.cell_ids) == ["cell_a", "cell_b"]
        assert_array_equal_safe(result.cell_values, [12.0, 21.0], "cell_values")