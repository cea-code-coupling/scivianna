...
import scivianna
from scivianna.constants import GEOMETRY, X, Y, Z
...
def get_panel(_, return_slaves=False) -> SplitLayout:
    ...
    med_1 = Panel2D(
        slave, name="MEDCoupling visualizer XY", displayed_field="INTEGRATED_POWER", u=X, v=Y
    )
    med_2 = Panel2D(
        slave, name="MEDCoupling visualizer XZ", displayed_field="INTEGRATED_POWER", u=X, v=Z
    )
    med_3 = Panel2D(
        slave, name="MEDCoupling visualizer YZ", displayed_field="INTEGRATED_POWER", u=Y, v=Z
    )
