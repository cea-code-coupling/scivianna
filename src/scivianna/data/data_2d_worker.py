...
class Data2DWorker:
    ...
    def has_changed(
        ...
        try:
            np.testing.assert_equal(
                np.array(self.data2d.cell_colors), np.array(self.data2d_save.cell_colors)
            )
            np.testing.assert_equal(
                np.array(self.data2d.cell_edge_colors), np.array(self.data2d_save.cell_edge_colors)
            )
            np.testing.assert_equal(
                np.array(self.data2d.cell_ids), np.array(self.data2d_save.cell_ids)
            )
            np.testing.assert_equal(
                np.array(self.data2d.cell_values), np.array(self.data2d_save.cell_values)
            )
    ...
    def set_colors(self, colors: np.ndarray) -> bool:
        ...
        """
        assert type(colors) in (
            np.ndarray,
            list,
        ), f"A numpy array is expected, type found {type(colors)}."
        colors = np.array(colors)
        assert len(colors.shape) == 2, f"A 2D numpy array is expected, shape found {colors.shape}."
        assert (
            colors.shape == np.array(self.data2d.cell_colors).shape
        ), f"We expect the same number of elements as in self.data2d.cell_colors, received shape {colors.shape} instead of {self.data2d.cell_colors}."
        assert (
            colors.flatten().max() <= 255
        ), f"The values must be lower than 255, found in array {colors.flatten().max()}."
        assert (
            colors.flatten().min() >= 0
        ), f"The values must be greater than 0, found in array {colors.flatten().min()}."

        self.data2d.cell_colors = np.array(colors)
        self.data2d.cell_edge_colors = np.array(get_edges_colors(colors))

        edge_colors = np.array(self.data2d.cell_edge_colors)

        if not isinstance(self.data2d.cell_values[0], str):
            edge_colors[:, 3] = np.where(
                np.isnan(np.array(self.data2d.cell_values)), 255, edge_colors[:, 3]
            )

        self.data2d.cell_edge_colors = np.array(edge_colors)
    ...
    def set_alphas(self, alphas: np.ndarray) -> bool:
        ...
        bool
            True if the given array is ok
        """
        assert type(alphas) in (
            np.ndarray,
            list,
        ), f"A numpy array is expected, type found {type(alphas)}."
        alphas = np.array(alphas)
        assert len(alphas.shape) == 1, f"A 1D numpy array is expected, shape found {alphas.shape}."
        assert alphas.shape[0] == len(
            self.data2d.cell_colors
        ), f"We expect the same number of elements as in self.data2d.cell_colors, received size {alphas.shape[0]} instead of {len(self.data2d.cell_colors)}."
        assert (
            alphas.max() <= 255
        ), f"The values must be lower than 255, found in array {alphas.max()}."
        assert (
            alphas.min() >= 0
        ), f"The values must be greater than 0, found in array {alphas.min()}."
