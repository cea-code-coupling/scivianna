...
try:
    ...
    raise ImportError(
        e,
        "To install the dependancies to use an agent, install scivianna using the command pip install scivianna[agent]",
    )

from scivianna.agent.llm_model import ai_server, call_llm
...
class Data2DWorker(Data2DWorker):
    ...
    def execute_code(self, code_to_execute: str):
        ...
        try:
            ...
            return (
                False,
                f"The code was executed, but the resulting Data2D is not valid. Following problem found {e}.",
            )
...
if __name__ == "__main__":
    ...
    data_2d, _ = med.compute_2D_data(
        X, Y, 0, 1, 0, 1, 0.0, None, {}, caller="AgentWorker"  # values not used
    )
