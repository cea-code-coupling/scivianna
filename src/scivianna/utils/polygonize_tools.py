...
class PolygonCoords:
    ...
    def __init__(
        self, x_coords: Union[List[float], np.ndarray], y_coords: Union[List[float], np.ndarray]
    ):
        ...
        if len(x_coords) != len(y_coords):
            raise ValueError(
                f"Given polygons coords must have the same length, found {len(x_coords)} and {len(y_coords)}"
            )
...
class PolygonElement:
    ...
    def __init__(self, exterior_polygon: PolygonCoords, holes: List[PolygonCoords], cell_id: str):
        """PolygonCoords object constructor.
...
def numpy_2D_array_to_polygons(
    ...
    shape_gen = (
        (shape(s), val) for s, val in rasterio.features.shapes(index_arr, transform=transform1)
    )
    ...
    for polygon, value_id in shape_gen:
        ...
        polygon_element_list.append(
            PolygonElement(
                exterior_polygon=PolygonCoords(
                    x_coords=np.array([vert[0] for vert in polygon.exterior.coords]),
