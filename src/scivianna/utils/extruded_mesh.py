...
class ExtrudedStructuredMesh(Geometry2D):
    ...
    def build_pyvista_geometry(
        ...
        extruded_cells = [
            mesh.copy(deep=True)
            .translate([0.0, 0.0, self.z_coords[i]])
            .extrude_trim(
        ...
        full_mesh = pv.merge(extruded_cells, merge_points=False)
        ...
        for cell_id in cell_per_id:
            ...
            for loop in rings:
                ...
                polys.append(
                    shapely.Polygon(
                        shell=np.array(
                            [mesh_slice.points[ids].dot(u), mesh_slice.points[ids].dot(v)]
                        ).T,
                        holes=[],
                    )
                )
