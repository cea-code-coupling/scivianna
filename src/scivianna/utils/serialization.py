...
def _get_extension_class(
    ext_name: str, interface: Type[GenericInterface], panel_type: str
) -> Optional[Type]:
    """
...
def _build_panel_metadata(
    panel_type: str, panel: Union["Panel1D", "Panel2D", "Panel3D"], interface_key: str
) -> Dict:
    """
...
def save_slave_to_file(
    slave: ComputeSlave, file_path: Union[str, Path], include_files: bool = True
) -> Path:
    """
...
def load_slave_from_file(
    file_path: Union[str, Path], interface_class: Type[GenericInterface], include_files: bool = True
) -> ComputeSlave:
    """
...
def save_panel2d_to_file(
    panel: "Panel2D", file_path: Union[str, Path], include_files: bool = True
) -> Path:
    """
...
def load_panel2d_from_file(file_path: Union[str, Path], include_files: bool = True) -> "Panel2D":
    """
...
def save_panel1d_to_file(
    panel: "Panel1D", file_path: Union[str, Path], include_files: bool = True
) -> Path:
    """
...
def load_panel1d_from_file(file_path: Union[str, Path], include_files: bool = True) -> "Panel1D":
    """
...
def save_panel3d_to_file(
    panel: "Panel3D", file_path: Union[str, Path], include_files: bool = True
) -> Path:
    """
...
def load_panel3d_from_file(file_path: Union[str, Path], include_files: bool = True) -> "Panel3D":
    """
...
def save_paneldatframe_to_file(
    panel: "PanelDataFrame", file_path: Union[str, Path], include_files: bool = True
) -> Path:
    """
...
def load_paneldatframe_from_file(
    file_path: Union[str, Path], include_files: bool = True
) -> "PanelDataFrame":
    """
...
def save_layout_to_zip(
    layout: "SplitLayout", file_path: Union[str, Path], include_files: bool = True
) -> Path:
    """
...
def load_layout_from_zip(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        split_item = _deserialize_split_item(
            layout_data["split_item"], layout_data, temp_path, include_files
        )

        # Create the layout instance
        layout = SplitLayout(split_item=split_item, additional_interfaces=additional_interfaces)
...
def _deserialize_split_item(
    data: Dict, full_data: Dict, temp_path: Path = None, include_files: bool = True
    ...
    Union[SplitItem, VisualizationPanel]
        Deserialized item
    """
    from scivianna.layout.split import SplitDirection, SplitItem
    ...
    if data["type"] == "SplitItem":
        ...
        panel_zip_path = _extract_panel_to_temp_zip(
            temp_path, panel_name, full_data, panel_json_data, extensions, extensions_data
        )
...
def restore_extensions_state(panel: "VisualizationPanel", split_item_data: Dict, full_data: Dict):
    """
    ...
    if split_item_data["type"] == "SplitItem":
        ...
        split_item_data["type"] == panel.__class__.__name__
        and split_item_data.get("panel_name") == panel.panel_name
    ):
...
def save_gridstack_to_zip(
    layout: "GridStackLayout", file_path: Union[str, Path], include_files: bool = True
) -> Path:
    """
        ...
        for panel_name, panel_info in layout_data["panels"].items():
            ...
            try:
                ...
                if panel_type == "Panel2D":
                    ...
                    panel = load_paneldatframe_from_file(
                        panel_zip_path, include_files=include_files
                    )
                else:  # Panel1D
                    panel = load_panel1d_from_file(panel_zip_path, include_files=include_files)
