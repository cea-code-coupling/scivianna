...
def interpolate_cmap_at_values(cmap_name: str, values: np.ndarray) -> np.ndarray:
    ...
    colors = (
        np.array([(int(c[1:3], 16), int(c[3:5], 16), int(c[5:7], 16)) for c in cmap]).astype(float)
        / 255
    )
...
beautiful_color_maps = {
    c: [list(e)[:3] for e in interpolate_cmap_at_values(c, np.arange(0, 1, 0.01))]
    for c in color_maps
}
