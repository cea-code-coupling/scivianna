...
class PolygonSorter:
    ...
    def sort_from_value(self, data: Data2D):
        ...
        assert len(data.cell_ids) == len(
            data.cell_colors
        ), "The Data2D object must have the same number of cell id and colors"
        assert len(data.cell_ids) == len(
            data.cell_edge_colors
        ), "The Data2D object must have the same number of cell id and edge colors"
        assert len(data.cell_values) == len(
            data.cell_colors
        ...
        if any(isinstance(item, str) for item in values_list):
            assert all(
                isinstance(item, str) for item in values_list
    ...
    def sort_list(self, data: Data2D):
        ...
        assert len(data.cell_values) == len(
            self.sort_indexes
        ), f"Given cell values list has a different length from the sorted indexes, respectively found {len(data.cell_values)} and {len(self.sort_indexes)}."
        assert len(data.cell_colors) == len(
            self.sort_indexes
        ), f"Given cell colors list has a different length from the sorted indexes, respectively found {len(data.cell_colors)} and {len(self.sort_indexes)}."
        assert len(data.cell_edge_colors) == len(
            self.sort_indexes
        ), f"Given cell edge colors list has a different length from the sorted indexes, respectively found {len(data.cell_edge_colors)} and {len(self.sort_indexes)}."
        assert len(data.cell_ids) == len(
            self.sort_indexes
        ), f"Given cell ID list has a different length from the sorted indexes, respectively found {len(data.cell_ids)} and {len(self.sort_indexes)}."
